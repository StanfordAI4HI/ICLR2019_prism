...
def _batch_potrf_lower(bmat):
    ...
    # for C in bmat.reshape((-1, n, n)):
    #     print (np.linalg.det(C.data.numpy()))
    # cholesky = torch.stack([torch.tensor(np.linalg.cholesky(C)) for C in bmat.reshape((-1, n, n))])
...
def _batch_mahalanobis_fast(L_inv, x):
    return (x.unsqueeze(-1) * L_inv).sum(-2).pow(2.0).sum(-1)
...
class MultivariateNormal(Distribution):
    ...
    @lazy_property
    def L_inv(self):
        flat_L = self.scale_tril.unsqueeze(0).reshape((-1,) + self.scale_tril.shape[-2:])
        return torch.stack([torch.inverse(Li.t()) for Li in flat_L]).view(self.scale_tril.shape)

    @lazy_property
    def log_det(self):
        return _batch_diag(self.scale_tril).abs().log().sum(-1)
    ...
    def log_prob(self, value):
        ...
        diff = value - self.loc
        M = _batch_mahalanobis_fast(self.L_inv, diff)
...
class MultivariateNormalFast(MultivariateNormal, TorchDistributionMixin):
